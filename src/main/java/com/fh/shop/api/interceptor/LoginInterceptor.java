...
public class LoginInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if(StringUtils.isEmpty(header)){
            throw new GlobalException(ResponseEnum.HANDER_IS_EXITS);
        }

        //判断头信息是否完整
        String[] split = header.split("\\.");
        if(split.length!=2){
            throw new GlobalException(ResponseEnum.HANDER_IS_MISS);
        }
        ...
        if(!newSignBase64.equals(signBase64)){
            throw new GlobalException(ResponseEnum.HANDER_IS_TAMPER);
        }
        //判断是否超时
        ...
        if(!exist){
            throw new GlobalException(ResponseEnum.HANDER_IS_TIME_OUT);
        }
